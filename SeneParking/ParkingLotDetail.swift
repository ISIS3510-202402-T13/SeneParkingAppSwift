...
struct ParkingLotDetailView: View {
    ...
    var body: some View {
        ...
        .onAppear{incrementWeekdayCounter2()}
    }
    ...
    func incrementWeekdayCounter2() {
        ...
        // Define the URL to fetch the document
        guard let fetchUrl = URL(string: "https://firestore.googleapis.com/v1/projects/seneparking-f457b/databases/(default)/documents/data/DetailAnalytics") else {
            print("Invalid URL for Firebase fetch")
            return
        }
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   ...
                   let currentValue = Int(currentValueString) {
                    ...
                    // Define the URL with document path for updating
                    guard let updateUrl = URL(string: "https://firestore.googleapis.com/v1/projects/seneparking-f457b/databases/(default)/documents/data/DetailAnalytics?updateMask.fieldPaths=\(weekday)") else {
                        print("Invalid URL for Firebase update")
                        return
                    }
